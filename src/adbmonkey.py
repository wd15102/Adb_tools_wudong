...
class AdbMonkey:
    ...
    def _adb_monkey(self):
        ...
        for k in self.monkey_cmd.split(','):
        ...
        while not self._stop_event.is_set() and time.time() < end_time:
            try:
                if self.temp_data.fd_num > 700 and config.fd_stop_var == '是':
                    logger.error(f'{self.device.adb.device_id} fd num > 700, adb monkey stop')
                    break
                before = time.time()
                view = activity = 0
    ...
    def start(self):
        ...
        self.adb_monkey_thread = threading.Thread(target=self._adb_monkey)
        self.adb_monkey_thread.start()
    ...
    def stop(self):
        ...
        if self.adb_monkey_thread.is_alive():
            self._stop_event.set()
            self.adb_monkey_thread.join(timeout=1)
            self.adb_monkey_thread = None
