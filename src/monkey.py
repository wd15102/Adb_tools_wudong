...
class Monkey:
    def __init__(self, device_id, package, temp_data, interval=5, timeout=888888888, collectors=None):
    ...
    def _monkey_cmd(self, log_file):
        random_number = random.randint(1, 100000)
        if self.pct_per.find(' -s ') == -1:
            self.pct_per = self.pct_per + f' -s {random_number}'
        cmd = f'monkey -p {self.package} -v -v -v --ignore-crashes --ignore-timeouts --ignore-security-exceptions --kill-process-after-error {self.pct_per}  99999999 > {log_file}'
        return cmd
    ...
    def _monkey_check(self, save_dir):
        ...
        self.start_time = Utils.get_current_underline_time()
        log_file = os.path.join(save_dir, 'monkey_%s.log' % self.start_time)
        cmd = self._monkey_cmd(log_file)
        self._log_process = self.device.adb.run_adb_shell_cmd(cmd, sync=False)
        time.sleep(30)

        while self.running:
            try:
                if self.temp_data.fd_num > 700 and config.fd_stop_var == '是':
                    logger.error('fd num > 700, kill process com.android.commands.monkey')
                    self.device.adb.kill_process("com.android.commands.monkey")
                    break
                current_package = self.device.adb.get_current_package()
                if not current_package:
                    time.sleep(2)
                    continue
                pid = self.device.adb.get_pid_from_package("com.android.commands.monkey")

                if current_package.find(self.package) == -1:
                    is_start += 1
                    if is_start > 4:
                        logger.debug(f'current package is {current_package}')
                        self.device.adb.run_adb_shell_cmd(f'kill -SIGSTOP {pid}')
                        time.sleep(10)
                        current_package = self.device.adb.get_current_package()
                        if current_package.find(self.package) == -1:
                            self.device.adb.start_activity(f'{self.package}/{self.main_activity}')
                            time.sleep(10)
                        self.device.adb.run_adb_shell_cmd(f'kill -SIGCONT {pid}')
                        is_start = 0
                    else:
                        time.sleep(2)
                        continue
                ...
                if pid and no_run_times < 6:
                    ...
                    no_run_times += 1
                    logger.info(f'monkey run pid:{pid}, no run times:{no_run_times}')
                    if no_run_times > 5:
...
if __name__ == "__main__":
    ...
    config.monkey_cmd = '--pct-majornav 45 --pct-nav 35 --pct-syskeys 5 --pct-motion 5 --pct-appswitch 10 --throttle 2000'
    TempData.result_path = r'D:\pythoncode\AdbTool\results'
    monkey = Monkey('', "com.mgtv.tv", TempData)
    monkey.start()
    time.sleep(360)
    monkey.stop()
