...
class CpuCollector:
    ...
    def _get_top_cmd(self):
        top_cmd = 'busybox top -b -n 1 -d %d' % self._interval
        ret = self.device.adb.run_adb_shell_cmd(top_cmd)
        if ret and 'not found' not in ret.lower():
            return top_cmd
    ...
    @staticmethod
    def get_filed_start_position(top_info, name):
        """
        根据字段名获取起始位置，busybox中数据排序不规律，无法使用空格进行切分
        :param top_info:
        :param name:
        :return:
        """
    ...
    def _busybox_parse_package(self, package, top_info):
        """
        通过busybox top命令获取测试包的cpu信息
        :return:
        """
        datetime = Utils.get_current_time()
        package_dict = {'datetime': datetime, 'package': package, 'pid': '', 'pid_cpu': ''}
        ...
        for line in line_list:
            if package in line:
                ...
                logger.debug("busybox parse package: %s, pid: %s" % (package_name, pid))
                if package == package_name:
                    start_position = self.get_filed_start_position(top_info, "%CPU")
                    if start_position is not None:
                        pkg_cpu_rate = line[start_position - 1: start_position + 5]
                        # Q601A：13980  1802 10044    R <  2128m 216%  18% com.hunantv.market
                        pkg_cpu_rate = re.search('[0-9]+(\.[0-9]+)?', pkg_cpu_rate).group()
                        package_dict['pid'] = pid
                        package_dict['pkg_cpu_rate'] = pkg_cpu_rate
                        break
        return package_dict
    ...
    @staticmethod
    def _busybox_parse_device(top_info):
        """
        通过busybox top命令获取测试设备的cpu信息
        :return:
        """
        # Mem: 1859460K used, 145404K free, 154564K shrd, 68660K buff, 939176K cached
        # CPU: 23.0% usr  4.6% sys  0.0% nic 72.3% idle  0.0% io  0.0% irq  0.0% sirq
        user_rate = ''
        sys_rate = ''
        idle_rate = ''
        total_rate = ''
        mem_used = ''
        free_used = ''
        re_cpu = r'CPU:\s*(.+?)%\s*usr\s*(.+?)%\s*sys\s*(.+?)%\s*nic\s*(.+?)%\s*idle\s*(.+?)%\s*io\s*(.+?)%\s*irq\s*(.+?)%\s*sirq'
        re_mem = r'Mem:\s*(.+?)K\s*used,\s*(.+?)K\s*free,\s*(.+?)K\s*shrd,\s*(.+?)K\s*buff,\s*(.+?)K\s*cached'
        cpu_match = re.compile(re_cpu).search(top_info)
        mem_match = re.compile(re_mem).search(top_info)

        if cpu_match:
            user_rate = cpu_match.group(1)
            sys_rate = cpu_match.group(2)
            idle_rate = cpu_match.group(4)
            total_rate = round(float(user_rate) + float(sys_rate), 1)
            if mem_match:
                mem_used = int(int(mem_match.group(1)) / 1024)
                mem_free = int(int(mem_match.group(2)) / 1024)
            logger.debug(f"user cpu: {user_rate}, sys cpu: {sys_rate}, idle cpu: {idle_rate}, total cpu: {total_rate}, "
                         f"mem used: {mem_used}")
        return dict(user_rate=user_rate, sys_rate=sys_rate, idle_rate=idle_rate, total_rate=total_rate,
    ...
    def _get_cpu_info(self):
        ...
        if 'busybox' in self._top_cmd:
            package_cpu = self._busybox_parse_package(self.package, top_info)
            device_cpu = self._busybox_parse_device(top_info)
        else:
    ...
    def _cpu_collect(self):
        ...
        mem_file = os.path.join(self.temp_data.result_path, 'mem.csv')
        mem_title = ["datetime", "used", "free"]
        try:
        ...
        while not self._stop_event.is_set() and time.time() < end_time:
            try:
                ...
                mem_used = cpu_dict.get('mem_used')
                mem_free = cpu_dict.get('mem_free')
                if pkg_cpu_rate is None:
                    logger.debug("package cpu get error")
                    continue
                current_time = Utils.get_current_time()
                cpu_list.extend(
                    [current_time, total_rate, user_rate, sys_rate, idle_rate, package, pkg_cpu_rate])

                logger.info('total_cpu_rate:%s%%, user:%s%%, system:%s%%, idle:%s%%, package_cpu:%s%%, mem_used:%sM' % (
                    total_rate, user_rate, sys_rate, idle_rate, pkg_cpu_rate, mem_used))
                ...
                try:
                    ...
                    with open(mem_file, 'a+', encoding="utf-8") as f:
                        csv.writer(f, lineterminator='\n').writerow([current_time, mem_used, mem_free])
                except RuntimeError as e:
                    logger.error(e)

                data = [self.temp_data.task_id, self.temp_data.platform, self.temp_data.model, self.temp_data.sver, self.temp_data.mac,
                self.temp_data.ver,self.temp_data.v_code, self.package, current_time, total_rate, user_rate, sys_rate, pkg_cpu_rate, mem_used]
                query = f"INSERT INTO cpu (task_id, platform, model,sver,mac,ver,v_code, package,datetime,total_rate,user_rate,sys_rate,pkg_cpu_rate,mem_used) " \
    ...
    def stop(self):
        ...
        if self.collect_cpu_thread.is_alive():
...
if __name__ == '__main__':
    ...
    cpu_monitor = CpuCollector('MAX0019111000070', 'com.mgtv.tv', TempData)
    cpu_monitor.start()
    time.sleep(60)
    cpu_monitor.stop()
