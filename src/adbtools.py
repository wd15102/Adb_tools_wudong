...
class ADB:
    ...
    def run_adb_exec_out_cmd(self, cmd, **kw):
        """
        使用adb exec-out执行命令
        :param cmd:
        :param kw:
        :return:
        """
        ret = self.run_adb_cmd('exec-out', '%s' % cmd, **kw)
        if ret is None:
            logger.error(u'adb cmd failed:%s ' % cmd)
        return ret
    ...
    def push_file(self, src_path, dst_path):
        ...
        if " " in src_path:
            src_path = '"' + src_path + '"'
        for i in range(3):
            result = self.run_adb_cmd('push', src_path, dst_path, timeout=600)
    ...
    def package_dumpheap(self, package, save_path):
        ...
        :return:
        """
        _, name = self.get_package_version(package)
        v_list = name.split('.')
        heap_name = f'{package}_dumpheap_{Utils.get_current_underline_time()}.hprof'
        remote_file = None
        local_file = os.path.join(save_path, heap_name)
        if len(v_list) >= 7 and 'TVAPP' in name and 'Pre' in name and (int(''.join(v_list[:3])) >= 70613):
            dumpheap_dir = f'/sdcard/Android/data/{package}/cache/HprofCache'
            self.run_adb_shell_cmd(f'rm -rf {dumpheap_dir}/*')

            # TVAPP7.0.7版本引入广播dump内存
            self.run_adb_shell_cmd('am broadcast -a com.mgtv.tv.ACTION_AUTO_DUMP_HPROF')
            time.sleep(5)
            for i in range(10):
                time.sleep(1)
                file_list = self.get_dir_file(dumpheap_dir)
                if len(file_list) == 0:
                    continue
                for file_name in file_list:
                    if not file_name.endswith('hprof'):
                        continue
                    remote_file = dumpheap_dir + '/' + file_name
                    break
                if remote_file is not None:
                    break
                else:
                    logger.debug(f'dump hprof file list:{file_list}')
            if remote_file is None:
                return 'auto dump hprof fail'

        else:
            remote_file = f'/data/local/tmp/{heap_name}'
            self.try_enable_debuggable()
            ret = self.run_adb_shell_cmd("am dumpheap %s %s" % (package, remote_file), timeout=180, error=True)
            if 'Process not debuggable' in ret:
                    logger.debug('Process not debuggable')
                    return 'Process not debuggable'
            elif 'Unknown process' in ret:
                logger.debug('Unknown process')
    ...
    def is_port_listening(self, port):
        result = self.run_adb_shell_cmd(f'netstat -anp |grep {port}')
        if not result:
            return False
        for line in result.splitlines():
    ...
    def get_port_process(self, port):
        ...
        try:
            ...
            print(f"get port process error: {e}")
            return None, None
    ...
    def kill_process(self, name):
        ...
        if pid_list:
            ret = self.run_adb_shell_cmd('kill ' + ' '.join([str(pid) for pid in pid_list]))
            if ret:
                ret = self.force_stop_package(name)
        else:
            logger.error(f'process is not exist {name}')
            ...
            try:
                ...
                if i >= 2:
                    ...
                    if ret and 'INSTALL_FAILED_ALREADY_EXISTS' in ret:
                        ret = 'Success'
        try:
            self.remove_file('/data/local/tmp/install.apk')
        except Exception as e:
            logger.debug(e)
        return ret
    ...
    def bugreport(self, save_path):
        ...
        :return:
        """
        if self.get_sdk_version() < 24:
            file_path = os.path.join(save_path, f'bugreport_{Utils.get_current_underline_time()}.txt')
            cmd = f'bugreport > {file_path}'
            result = self.run_adb_cmd(cmd, timeout=300)
        else:
            file_path = os.path.join(save_path, f'bugreport_{Utils.get_current_underline_time()}.zip')
            result = self.run_adb_cmd(f'bugreport {file_path}', timeout=300)
        return result

    def get_package_size(self, package):
        ret = self.run_adb_shell_cmd(f'pm list package -f {package}')
        package_list = ret.split()
        for item in package_list:
            match = re.match(f'package:(.+?)={package}$', item)
            if match:
                path = match.group(1)
                return self.get_path_size(path)

    def get_path_size(self, path):
        ret = self.run_adb_shell_cmd(f'busybox du -s {path}')
        ret_list = ret.split()
        if len(ret_list) > 0:
            size = ret_list[0]
            if size.isdigit():
...
if __name__ == '__main__':
    dev = AdbUtils('MAX0019071000087')
    ret = dev.adb.local_ip()
    print(ret)