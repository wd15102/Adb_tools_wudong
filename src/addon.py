...
from config import root_path, server_ip
# from mitmproxy.script import concurrent  # 新版 mitmproxy 不再支持
...
class DpProxy:
    ...
    def get_cipher_config(cls, data, codec):
        ...
        try:
            res = requests.post(e_pack, headers=cls.headers, data=form)
    ...
    @classmethod
    def get_decrypt_config(cls, data, codec):
        url = 'http://inottuni.ng.imgo.tv/v1/inside/decrypto'
        form = dict(mtype=2, codec=codec, pack=data)

        try:
            res = requests.post(url, headers=cls.headers, data=form)
            json_data = json.loads(res.text)
            if json_data.get('code') == 200:
                plaintext = json_data.get('data')
                return plaintext
    ...
    def ott_start_handle(cls, res, flow, _from):
        ...
        # 新增使用config接口明文的全部返回参数且兼容历史数据，避免忘记这里的规则导致配置不生效
        res_json = json.loads(res)
        new_res = res_json.get('data')
        if new_res:
            res = json.dumps(new_res, ensure_ascii=False, separators=(',', ':'))

        if cls.ott_cfg:
            for cfg in cls.ott_cfg:
                res = json.dumps(json.loads(res), ensure_ascii=False, separators=(',', ':'))
                res = cls.sub_json(cfg[0], cfg[1], res)

        if flow.response.text.find('apiAddr') != -1:
    ...
    def get_field_value(key: str, text: str):
        ...
        pattern = r'"%s"\s?:\s?("[^"]+"|\d+)' % key
        # lob字段中的转义符删除，URL中有转义符不能删除
        if text.find('\\"') != -1:
            text = text.replace('\\', '')
    ...
    @staticmethod
    def hide_common_fields(flow, remove_fields, miss=True, empty=True):
        """
        根据 remove_fields 删除或补充请求中的字段，兼容：
        ...
        def process_dict(data_dict):
            new_data = dict(data_dict)  # 复制一份，避免边遍历边删除
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    for f in remove_fields:
                        if f not in data and miss:
                            data[f] = "error"  # 字段不存在 → 补充
                        else:
                            data.pop(f, None)  # 删除字段
        ...
        if method == "POST" and "x-www-form-urlencoded" in content_type:
            form = dict(flow.request.urlencoded_form.items())
            if empty:
                form = process_dict(form)

            for f in remove_fields:
                if f not in form and miss:
                    form[f] = "error"
                else:
                    form.pop(f, None)
        ...
        # ========== 3. GET query 参数 ==========
        if method == "GET":
            query = dict(flow.request.query.items())
            if empty:
                query = process_dict(query)

            for f in remove_fields:
                if f not in query and miss:
                    query[f] = "error"
                else:
                    query.pop(f, None)
    ...
    def sort_parameters(flow):
        if '127.0.0.1' in flow.request.host or server_ip in flow.request.host:
            # 重复排序
            return
        sorted_params = []
                ...
                for key, value in to.items():
                    if key and value:
                        if key == 'headers':
                            ...
                            # 减少公共字段显示
                            if self.config.get('common_hide'):
                                miss = self.config.get('common_miss')
                                remove_fields = list(self.config.get(self.config.get('project')).get('common').keys())
                                self.hide_common_fields(flow, remove_fields, miss)
                            # 兼容apm上报
                            if 's.gif' in domain:
                                data = dict(sorted_params).get('data')
                                ...
                                for k, v in data_dict.items():
                                    ...
                                    if k == 'event_name':
                                        logtype = v
                                        new_list.append(('logtype', v))
                                        continue
                                    new_list.append((k, v))
                                sorted_params = sorted(new_list)
                            field_dict = dict(sorted_params)
                            if self.config.get('integer'):
                    ...
                    if key == 'dest':
                        ...
                        if value == '1':
                            ...
                            try:
                                ...
                                codec = self.get_request_params(flow.request, 'codec')
                                if codec:
                                    data = self.get_decrypt_config(data, codec)
                                else:
                                    data = json.dumps(data, ensure_ascii=False, separators=(',', ':'))
                                data = re.sub('"isEncrypt":"1"', '"isEncrypt":"0"', data, flags=re.S)
                                data = re.sub('https', 'http', data, flags=re.S)
                                if codec:
                                    data = self.get_cipher_config(data, codec)
                                else:
                                    data = json.loads(data)
                                json_obj['data'] = data
                                response_text = json.dumps(json_obj, ensure_ascii=False, separators=(',', ':'))
                                flow.response.set_text(response_text)
                            except json.JSONDecodeError:
                                logger.error(f'local map json error:{text}')
                    else:
