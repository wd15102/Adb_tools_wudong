...
class DataReport:
        ...
        while not self._stop_event.is_set():
            ...
            if len(self.report_list) > 0:
                ...
                if wait_pv_time - time.time() > 0:
                    ...
                    try:
                        requests.post(f'http://{server_ip}/need_pv', json={}, proxies=proxies, timeout=3)
                ...
                # pv测试前，可将上一次存的lastp、fpid赋值
                if logtype == 'pv':
                    self.latest_fields['lastp'] = self.lastp
                    self.latest_fields['lastpid'] = self.fpid
                    self.latest_fields['fpn'] = self.lastp
                    self.latest_fields['fpid'] = self.fpid


                text = self.field_batch(test_field, gain_field)
                try:
                    requests.post(f'http://{server_ip}/{logtype}', json=text, proxies=proxies, timeout=3)
                except Exception as e:
                    logger.debug(e)

                # pv测试完毕后，cntp、lastp已变化，需要再修改一遍数据
                if logtype == 'pv':
                    ...
                    # 按规范当前Pv的cntp、cpid为下一次pv的lastp、lastpid
                    self.lastp = test_field.get('cntp', '')
                    self.fpid = test_field.get('cpid', '')
    ...
    def inter_compare(key, input_data, gain_field):
        ...
        assert text == value or value == '0', f'{value} ?'
    ...
    def cpid_compare(self, key, input_data, gain_field):
        ...
        text = input_data.get(key)
        if project == 'OTT' and logtype in ['cv', 'click']:
            return
        elif cntp in ['I', 'IX', 'dj_play']:
    ...
    def lob_compare(self, out_data, input_data, gain_field, value):
        ...
        for k, v in query_dict.items():
            ...
            if isinstance(field_value, str) and getattr(self, field_value, None) is not None:
                try:
